...
const KakaoMapScript = ({ positions }) => {
	...
	// 중복된 마커 배열
	let dupPositions = [], tempPositions = [];
	positions.forEach(position => {
		if (tempPositions.some(pos => pos.building_name === position.building_name)) {
	...
	// 중복되지 않는 배열 display
	positions.forEach(position => {
		if (dupPositions.some(pos => pos.building_name === position.building_name)) {
			return; 
		}
		...
		const handleMarkerClick = () => {
				...
				ReactDOM.render(<Modal isOpen={true} closeModal={() => ReactDOM.unmountComponentAtNode(modalRoot)} positionId={position.buildingId} classId={position.classID} />, modalRoot);
		};
	...
	dupPositions.forEach(position => {
		...
		const handleMarkerClick = () => {
			// 모달 팝업 열기
			const modalRoot = document.getElementById('modal-root');
			ReactDOM.render(<DupModal isOpen={true} closeModal={() => ReactDOM.unmountComponentAtNode(modalRoot)} positionId={position.buildingId} classId={position.classID} positions={positions} />, modalRoot);
		};
